...
if tool_selection == "Pipe Network Builder":
    ...
    with col2:
        ...
        max_penalty = st.number_input("Max Penalty (K)", min_value=0.0, max_value=6.0, value=1.0)
    ...
    if ID_mm is not None:
        ...
        density_super_foroil = RefrigerantDensities().get_density(refrigerant, T_evap - max_penalty + 273.15, min(max(superheat_K, 5), 30) + max_penalty)
