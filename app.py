...
if tool_selection == "Pipe Network Builder":
    ...
    with col1:
        ...
        # --- Callbacks: only mutate session_state here ---
        def on_change_cond():
            # Do NOT pull minliq down; just ensure cond ≥ minliq
            if ss.cond_temp < ss.minliq_temp:
                ss.cond_temp = ss.minliq_temp

        def on_change_minliq():
            # Keep minliq ≤ cond
            if ss.minliq_temp > ss.cond_temp:
                ss.minliq_temp = ss.cond_temp
            # Downstream clamp: evap ≤ minliq
            if ss.evap_temp > ss.minliq_temp:
                ss.evap_temp = ss.minliq_temp
        ...
        # --- Widgets with dynamic, inclusive bounds ---
        st.number_input(
            "Min Liquid Temperature (°C)",
            min_value=minliq_min,
            max_value=min(ss.get("cond_temp", cond_default), minliq_max),
            step=1.0,
            key="minliq_temp",
        ...
        st.number_input(
            ...
            min_value=max(cond_min, ss.get("minliq_temp", minliq_default)),
            max_value=cond_max,
            step=1.0,
            key="cond_temp",
        ...
        st.number_input(
            ...
            min_value=evap_min,
            max_value=min(ss.get("minliq_temp", minliq_default), evap_max),
            step=1.0,
            key="evap_temp",
