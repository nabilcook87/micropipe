...
if tool_selection == "Pipe Network Builder":
    ...
    if ID_mm is not None:
        ...
        oil_density_sat = (-0.0000759301739563512 * (T_evap ** 3)) + (0.00704394974196987 * (T_evap ** 2)) - (0.239344055739792 * T_evap) + 962.932850216485
        oil_density_super = (-0.0000759301739563512 * ((T_evap + min(max(superheat_K, 5), 30)) ** 3)) + (0.00704394974196987 * ((T_evap + min(max(superheat_K, 5), 30)) ** 2)) - (0.239344055739792 * (T_evap + min(max(superheat_K, 5), 30))) + 962.932850216485
        oil_density = (oil_density_sat + oil_density_super) / 2
        MinMassFlux = (0.895 ** 2) * ((density_foroil * 9.81 * ID_m * (oil_density - density_foroil)) ** 0.5)
