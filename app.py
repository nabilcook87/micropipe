...
if tool_selection == "Pipe Network Builder":
    ...
    if ID_mm is not None:
        ...
        oil_density_sat = (0.0611904761904762 * (T_evap ** 2)) + (1.37142857142854 * T_evap) + 928.178571428568
        oil_density_super = (0.0611904761904762 * ((T_evap + min(max(superheat_K, 5), 30)) ** 2)) + (1.37142857142854 * (T_evap + min(max(superheat_K, 5), 30))) + 928.178571428568
        oil_density = (oil_density_sat + oil_density_super) / 2
        
        if refrigerant == "R404A": jg_half = 0.9065
        if refrigerant == "R134a": jg_half = 0.9155
