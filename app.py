...
if tool_selection == "Pipe Network Builder":
    ...
    with col1:
        ...
        if refrigerant == "R23": evaporating_temp = st.number_input("Evaporating Temperature (°C)", min_value=-100.0, max_value=min(-20.0, minliq_temp), value=-80.0, step=1.0)
        elif refrigerant == "R508B": evaporating_temp = st.number_input("Evaporating Temperature (°C)", min_value=-100.0, max_value=min(-20.0, minliq_temp), value=-80.0, step=1.0)
        elif refrigerant == "R744": evaporating_temp = st.number_input("Evaporating Temperature (°C)", min_value=-50.0, max_value=min(20.0, minliq_temp), value=-10.0, step=1.0)
        else: evaporating_temp = st.number_input("Evaporating Temperature (°C)", min_value=-50.0, max_value=min(30.0, minliq_temp), value=-10.0, step=1.0)
        if refrigerant == "R23": condensing_temp = st.number_input("Max Liquid Temperature (°C)", min_value=-100.0, max_value=10.0, value=-30.0, step=1.0)
