...
if tool_selection == "Pipe Network Builder":
    ...
    if mode == "Wet Suction":
        ...
        st.write("Refrigerant:", refrigerant)
        st.write("Selected Pipe Material:", selected_material)
        st.write("Selected Nominal Pipe Size (inch):", selected_size)
        st.write("Selected Gauge:", ss.get("gauge", None))
        st.write("Evaporator Capacity (kW):", evap_capacity_kw)
        st.write("Liquid Overfeed Quantity (%):", liq_oq)
        st.write("Wet Suction Penalty Factor:", WetSucPenaltyFactor)
        st.write("Max Penalty (K):", max_penalty)
        st.write("Evaporating Temperature (°C):", evaporating_temp)
        st.write("Pipe Length (m):", L)
        st.write("Long Radius Bends:", LRB)
        st.write("Short Radius Bends:", SRB)
        st.write("45° Bends:", _45)
        st.write("Machine Bends:", MAC)
        st.write("P-Traps:", ptrap)
        st.write("U-Bends:", ubend)
        st.write("Ball Valves:", ball)
        st.write("Globe Valves:", globe)
        st.write("Additional PLF:", PLF)
        st.write("Pipe Material Roughness (eps):", eps)
        st.write("Internal Diameter (mm):", ID_mm)
        st.write("Internal Diameter (m):", D_int)
        st.write("Pipe Cross-Section Area (m²):", A_total)
        st.write("Evaporating Temperature Used (T_evap):", T_evap)
        st.write("Liquid Enthalpy (h_in):", h_in)
        st.write("Vapor Enthalpy (h_out):", h_out)
        st.write("Delta h (deltah):", deltah)
        st.write("Base Massflow (kg/s):", base_massflow)
        st.write("Overfeed Ratio:", overfeed_ratio)
        st.write("Vapour Mass Flow (m_g):", m_g)
        st.write("Liquid Mass Flow (m_l):", m_l)
        st.write("Total Mass Flow (m_gplusl):", m_gplusl)
        st.write("Liquid Density (d_liq):", d_liq)
        st.write("Vapor Density (d_vap):", d_vap)
        st.write("Liquid Viscosity (v_liq):", v_liq)
        st.write("Vapor Viscosity (v_vap):", v_vap)
        st.write("Liquid Volumetric Flow (Q_l):", Q_l)
        st.write("Vapor Volumetric Flow (Q_g):", Q_g)
        st.write("Liquid Ratio:", liquid_ratio)
        st.write("Gas Flow Area (A_gas):", A_gas)
        st.write("Gas Velocity (m/s):", gas_velocity)
        st.write("Hydraulic Diameter (D_h):", D_h)
        st.write("Reynolds Number:", Re)
        st.write("Friction Factor (f):", f)
        st.write("Dynamic Pressure (dyn):", dyn)
        st.write("Pipe ΔP (dp_pipe):", dp_pipe)
        st.write("Fittings ΔP (dp_fittings):", dp_fittings)
        st.write("Valves ΔP (dp_valves):", dp_valves)
        st.write("PLF ΔP (dp_plf):", dp_plf)
        st.write("Wet Suction Factor:", WetSucFactor)
        st.write("Pipe ΔP WS:", dp_pipe_ws)
        st.write("Fittings ΔP WS:", dp_fittings_ws)
        st.write("Valves ΔP WS:", dp_valves_ws)
        st.write("PLF ΔP WS:", dp_plf_ws)
        st.write("Total ΔP WS (dp_total_ws):", dp_total_ws)
        st.write("Evaporating Pressure:", evappres)
        st.write("Post Circulation Pressure:", postcirc)
        st.write("Post Circulation Temperature:", postcirctemp)
        st.write("Temperature Penalty (dt):", dt)
